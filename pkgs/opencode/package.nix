{
  lib,
  stdenv, # <-- Adicionado para compilação/patching
  stdenvNoCC,
  fetchurl,
  unzip,
  autoPatchelfHook, # <-- Adicionado para consertar binários no NixOS
  fetchFromGitHub,
  fzf,
  makeBinaryWrapper,
  models-dev,
...
let
  bun-baseline = stdenv.mkDerivation rec {
    pname = "bun-baseline";
    version = "1.3.12";
    ...
    # O Bun precisa da biblioteca padrão de C++ para rodar
    buildInputs = [
      stdenv.cc.cc.lib
    ];
    ...
    installPhase = ''
      mkdir -p $out/bin
      # O zip extrai uma pasta chamada 'bun-linux-x64-baseline', então procuramos o binário dentro dela
      install -Dm755 */bun $out/bin/bun
    '';

    # Isso resolve o aviso "evaluation warning: getExe"
...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/bin
    makeWrapper ${lib.getExe bun-baseline} $out/bin/opencode \
      --add-flags "run" \
